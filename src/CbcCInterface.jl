...
mutable struct CbcModel
    p::Ptr{Nothing}
    function CbcModel()
        p = @cbc_ccall newModel Ptr{Nothing} ()
        prob = new(p)
...
function deleteModel(prob::CbcModel)
    ...
    @cbc_ccall deleteModel Nothing (Ptr{Nothing},) prob.p
    prob.p = C_NULL
    return
end
...
macro getproperty(T, name)
    @eval function ($name)(prob::CbcModel)
        check_problem(prob)
        @cbc_ccall $name $T (Ptr{Nothing},) prob.p
    end
end
...
function loadProblem(prob::CbcModel,
    ...
    @cbc_ccall loadProblem Nothing (Ptr{Nothing}, Int32, Int32, Ptr{CoinBigIndex},
...
function readMps(prob::CbcModel, filename::String)
    ...
    @cbc_ccall readMps Cint (Ptr{Nothing}, Ptr{UInt8}) prob.p filename
end

function writeMps(prob::CbcModel, filename::String)
    check_problem(prob)
    @assert isascii(filename)
    @cbc_ccall writeMps Cint (Ptr{Nothing}, Ptr{UInt8}) prob.p filename
end

function setInitialSolution(prob::CbcModel, array::Vector{Float64})
    check_problem(prob)
    @cbc_ccall setInitialSolution Nothing (Ptr{Nothing}, Ptr{Float64}) prob.p array
end

function problemName(prob::CbcModel)
    check_problem(prob)
    a = Array(UInt8, 100)
    @cbc_ccall problemName Nothing (Ptr{Nothing},Cint,Ptr{UInt8}) prob.p 100 a
    return string(a)
end

#function setProblemName(prob::CbcModel)
#    check_problem(prob)
#    @cbc_ccall setProblemName Cint (Ptr{Nothing},Ptr{UInt8}) prob.p bytestring(a)
#end

function getNumElements(prob::CbcModel)
    check_problem(prob)
    @cbc_ccall getNumElements Cint (Ptr{Nothing},) prob.p
end

function getVectorStarts(prob::CbcModel)
    check_problem(prob)
    p = @cbc_ccall getVectorStarts Ptr{CoinBigIndex} (Ptr{Nothing},) prob.p
...
function getIndices(prob::CbcModel)
    check_problem(prob)
    p = @cbc_ccall getIndices Ptr{Cint} (Ptr{Nothing},) prob.p
...
function getElements(prob::CbcModel)
    check_problem(prob)
    p = @cbc_ccall getElements Ptr{Float64} (Ptr{Nothing},) prob.p
    nnz = Int(getNumElements(prob))
    return copy(unsafe_wrap(Array,p,(nnz,)))
end
...
function setObjSense(prob::CbcModel, sense)
    check_problem(prob)
    @cbc_ccall setObjSense Nothing (Ptr{Nothing}, Float64) prob.p sense
end
...
for s in (:getRowLower, :getRowUpper, :getRowActivity)
    @eval function ($s)(prob::CbcModel)
        ...
        p = @cbc_ccall $s Ptr{Float64} (Ptr{Nothing},) prob.p
...
for s in (:setRowUpper, :setRowLower, :setObjCoeff, :setColLower, :setColUpper)
    @eval function($s)(prob::CbcModel, index::Integer, value::Float64)
        check_problem(prob)
        @cbc_ccall $s Nothing (Ptr{Nothing}, Cint, Float64) prob.p index value
    end
end

function isInteger(prob::CbcModel, index::Integer)
    check_problem(prob)
    v = @cbc_ccall isInteger Cint (Ptr{Nothing},Cint) prob.p index
    return v == 1
end

function setContinuous(prob::CbcModel, index::Integer)
    check_problem(prob)
    @cbc_ccall setContinuous Nothing (Ptr{Nothing},Cint) prob.p index
end

function setInteger(prob::CbcModel, index::Integer)
    check_problem(prob)
    @cbc_ccall setInteger Nothing (Ptr{Nothing},Cint) prob.p index
end

function Base.copy(prob::CbcModel)
    p = @cbc_ccall clone Ptr{Nothing} (Ptr{Nothing},) prob.p
    prob = CbcModel(p)
    finalizer(prob, deleteModel)
    return prob
end
...
function setParameter(prob::CbcModel, name::String, value::String)
    ...
    @cbc_ccall setParameter Nothing (Ptr{Nothing},Ptr{UInt8},Ptr{UInt8}) prob.p name value
end

# TODO: registerCallBack clearCallBack

function solve(prob::CbcModel)
    @cbc_ccall solve Cint (Ptr{Nothing},) prob.p
end
...
for s in (:isAbandoned, :isProvenOptimal, :isProvenInfeasible, :isContinuousUnbounded,
    ...
    @eval function ($s)(prob::CbcModel)
        check_problem(prob)
        v = @cbc_ccall $s Cint (Ptr{Nothing},) prob.p
        return v != 0
    end
end
...
function addSOS(prob::CbcModel, numRows::Integer, rowStarts::Vector{Cint},
    ...
    @cbc_ccall(addSOS,Nothing,(Ptr{Nothing}, Cint, Ptr{Cint}, Ptr{Cint},
